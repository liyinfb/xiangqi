...
describe("Comprehensive AI Strength Tests", () => {
  ...
  it("should not move cannon into chariot's attack line", () => {
    ...
    // Should not move cannon to column 7 where chariot can capture
    if (move?.from.row === 2 && move?.from.col === 7) {
      expect(move.to.col).not.toBe(7); // Don't stay on column 7 moving toward chariot
    }
  }, 15000);
